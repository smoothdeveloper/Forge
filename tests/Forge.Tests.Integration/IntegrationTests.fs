...
let getRelativePath (dir: DirectoryInfo) (fsInfo: FileSystemInfo) = 
    fsInfo.FullName.Substring(dir.FullName.Length)
...
let getFilesDict (d: DirectoryInfo) =
    d.GetFiles() |> Seq.map (fun f -> f.Name, f) |> dict

let getDirectoriesDict (d: DirectoryInfo) =
    d.GetDirectories() |> Seq.map (fun d -> d.Name, d) |> dict

let compareFolders (d1: DirectoryInfo) (d2: DirectoryInfo) =
  
    let rec compareFolders root d1 d2 =
    
        let files1, files2 = getFilesDict d1, getFilesDict d2
        let dirs1, dirs2 = getDirectoriesDict d1, getDirectoriesDict d2
